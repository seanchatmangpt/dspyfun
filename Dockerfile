...
ENV VIRTUAL_ENV /opt/dspyfun-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/dspyfun/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/dspyfun/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/dspyfun/ && touch src/dspyfun/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/dspyfun/
...
# Expose the app.
ENTRYPOINT ["/opt/dspyfun-env/bin/dspyfun"]
CMD []
